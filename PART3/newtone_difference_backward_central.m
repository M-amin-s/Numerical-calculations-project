function output = newtone_difference_backward_central(points,  x,  precision, showFigure)
...
for i = 1:n
    if(i >1)
        func = sprintf('%s+%d', func, table(floor((i+n)/2), i+1));
        t1 = sprintf('%%s+%%.%df', precision);
        func_clean = sprintf(t1, func_clean, table(floor((i+n)/2), i+1));
        %adding the (r -i) part
        for j = 1:i-1
            if(mod(j+n-1, 2)==1)
                mul = -1;
            ...
            func = sprintf('%s*(r+%d)', func, mul * ceil((j-1)/2));
            if(j==1)
                t1 = sprintf('%%s(r)');
                func_clean = sprintf(t1, func_clean);
            else
                t1 = sprintf('%%s(r+%%.0f)');
                func_clean = sprintf(t1, func_clean, mul * ceil((j-1)/2));
            end
        end
    else
        func = sprintf('%d', table(floor((i+n)/2), i+1));
        t1 = sprintf('%%.%df', precision);
        func_clean = sprintf(t1, table(floor((i+n)/2), i+1));
    end
...
%show the func
%str = sprintf('%s  = %s', func_clean) ;
str = sprintf('%s', func_clean) ;
