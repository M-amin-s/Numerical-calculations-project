function output = newtone_difference_forward_central(points,  x,  precision, showFigure)
...
for i = 1:n
    if(i >1)
        func = sprintf('%s+%d', func, table(ceil((i+n)/2), i+1));
        t1 = sprintf('%%s+%%.%df', precision);
        func_clean = sprintf(t1, func_clean, table(ceil((i+n)/2), i+1));
        %adding the (r -i) part
        for j = 1:i-1
            if(mod(j+n-1, 2)==1)
                mul = -1;
        ...
        func = sprintf('%d', table(ceil((i+n)/2), i+1));
        t1 = sprintf('%%.%df', precision);
        func_clean = sprintf(t1, table(ceil((i+n)/2), i+1));
    end
